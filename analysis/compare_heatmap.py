...
comp_file = "final_results/lstm_2_200_crf_ontonotes_sd_-1_dep_none_elmo_none_sgd_gate_0_base_-1_epoch_100_lr_0.01.results"
comp_insts = read_results(comp_file)  ##change inst.output -> inst.prediction
...
### add grandchild relation as well.
for inst in insts:
    for head, dep in zip(inst.input.heads, inst.input.dep_labels):
        if head == -1:
            continue
        dep_labels.add(dep+", " + inst.input.dep_labels[head])
print(len(dep_labels), dep_labels)
###
...
for inst, comp_inst in zip(insts,comp_insts):
    for label, dep, gold, comp_label, head in zip(inst.prediction, inst.input.dep_labels, inst.output, comp_inst.prediction, inst.input.heads):
        if gold == "O":
            continue
        if label == "O":
            continue
        if label == gold and label != comp_label:
            ent_dep_mat[ent2idx[label[2:]]][dep2idx[dep]] += 1
            if head != -1:
                if inst.output[head] == inst.prediction[head] and inst.output[head] != comp_inst.prediction[head]:
                    ent_dep_mat[ent2idx[label[2:]]][dep2idx[dep+", " + inst.input.dep_labels[head]]] += 1
